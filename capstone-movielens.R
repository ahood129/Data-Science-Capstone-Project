...
## Create edx dataset and final_holdout_test dataset-----------------------------
...
# Create text theme for ggplot objects in the report
plot_theme <-
...
edx %>% ggplot(aes(rating)) +
  geom_histogram(binwidth = 0.2, color = "black", fill = "#4A90E2") +
...
edx %>% group_by(movieId) %>%
  ...
  geom_histogram(bins = 30, color = "black", fill = "#4A90E2") +
...
# Separate genres and rank them by the total number of ratings in the edx dataset
...
edx %>% group_by(genres) %>%
  ...
  labs(x = "Genre combinations", y = "Average Rating", caption = "Source: edx dataset") + plot_theme
...
##Methods------------------------------------------------------------------------
# Partition edx dataset into train and test sets
...
###Develop, train and test various iterations of the algorithm-----------

# Create table with target RMSE for full credit
rmse_objective <- 0.86490
rmse_results <-
  data.frame(Method = "Project objective",
             RMSE = "0.86490",
             Difference = "-")
...
###Update final_holdout_test dataset to reflect changes to edx dataset (year, date of review) and run final test------

final_holdout_test <-
  final_holdout_test %>% mutate(review_date = round_date(as_datetime(timestamp), unit = "week"))
final_holdout_test <-
  final_holdout_test %>% mutate(review_date = as_date(review_date))
final_holdout_test <-
  final_holdout_test %>% mutate(title = str_trim(title)) %>%
  extract(title,
          c("title_temp", "year"),
          regex = "^(.*) \\(([0-9 \\-]*)\\)$",
          remove = F) %>%
...
b_r <- edx %>%
  ...
  left_join(b_y, by = "year") %>%
  group_by(review_date) %>%
  summarise(b_r = sum(rating - b_i - b_u - b_g - b_y - mu_hat) / (n() +
                                                                    lambda))

# Predict ratings in final_holdout_test set using final model
...
##Results------------------------------------------------------------------------
###Plot disctribution of each of the effects modeled and print results in a table-----

# Add simple average RMSE result to table
rmse_results <-
  rmse_results %>% rbind(c(
    "Simple average",
    round(simple_rmse, 5),
    round(simple_rmse - rmse_objective, 5)
  ))

# Plot movie effects distribution
movie_avgs %>%
  ggplot(aes(b_i)) +
  geom_histogram(bins = 10, color = "black", fill = "#4A90E2") +
...
rmse_results <-
  rmse_results %>% rbind(c(
    "Movie, User and Genre effects (b_g)",
    round(genre_rmse, 5),
...
rmse_results <-
  rmse_results %>% rbind(c(
    "Movie, User, Genre and Year effects (b_y)",
    round(year_rmse, 5),
...
rmse_results <-
  rmse_results %>% rbind(c(
    "Movie, User, Genre, Year and Review Date effects (b_r)",
    round(review_rmse, 5),
...
rmse_results <-
  rmse_results %>% rbind(c(
    "Regularised Movie, User, Genre, Year and Review Date effects",
    round(regularised_rmse, 5),
    format(round(regularised_rmse - rmse_objective, 5), scientific = F)
  ))

###Final Model RMSE--------
# Create table to show final RMSE result with final_holdout_test dataset against the full-credit RMSE goal
final_results <-
  data.frame(Method = "Project Goal:",
             RMSE = "0.86490") %>%
  rbind(c("Final Model:",
          round(valid_rmse, 5)))

# Print the models final RMSE
print(final_results)
